...
Grid-search `3072 combinations` (`stein_shrink × w_linzer × sigma_int × sigma_slope`), folds:
- Treino: 2018 + outros eventos políticos → Teste: 2022 (54 eventos)
- Treino: 2022 + outros → Teste: 2018 (54 eventos)

**Configuração ótima do grid base (pré-MRP state):**
```
stein_shrink = 0.05
w_linzer     = 0.50
sigma_int    = 3.0
...
Esse grid é o rastreador amplo de hiperparâmetros. O ponto operacional publicado usa MRP state + evolução (`v1.4-evo`), porque vence em decisão winner/loser no painel year-fold completo.
...
| **Média ponderada** | **394** | **97.21%** | 0.105 |
...
**Autoresearch rigor panel v1.4-evo**: AUC 0.992, MCC 0.944, Brier skill vs climatologia 58.2%, ECE 0.261, Wilson 95% da acurácia 95.1–98.4%. McNemar confirma o ganho de decisão: +21 acertos líquidos vs baseline sem MRP.

**Autoevolução v1.4-evo**: `scripts/evolve_political_model.py --apply` promoveu `sigma_slope=0.005` por mutação elitista, crossover e hill-climb. Gates obrigatórios: acurácia e MCC não podem cair, AUC não pode cair materialmente, Brier não pode piorar materialmente e o score composto precisa subir. Resultado: score 4.4343 → 4.4351, AUC 0.99157 → 0.99163, Brier 0.10484 → 0.10460.
...
Best config v1.4-evo: `shrink=0.40, w_linzer=0.70, sigma_int=3.0, sigma_slope=0.005, w_state=0.36`. Persistido em `data/political_best_config.json`.
...
### 5.3 Autoevolução com gates

```bash
python3 scripts/evolve_political_model.py --apply
# Resultados -> data/political_evolution.json
# Promoção -> data/political_best_config.json, apenas se todos os gates passarem
```
...
| GET | `/api/v1/politica/evolution` | relatório de autoevolução + gates |
...
Snapshot 2026-05-14 (143 dias antes da eleição):

### Presidencial (1º turno)
| # | Candidato | Partido | Lead | P(vencer) |
|---:|---|---|---:|---:|
| 1 | Lula | PT | 0 | **36.1%** |
| 2 | Tarcísio | REP | -8 | 18.4% |
| 3 | Ratinho Jr | PSD | -22 | 18.0% |
| 4 | Zema | NOVO | -25 | 18.0% |
| 5 | Boulos | PSOL | -33 | 9.6% |

Lula lidera a primeira rodada normalizada; Bolsonaro segue filtrado por inelegibilidade TSE até 2030.

### Governadores (top UFs)
- **SP**: Tarcísio 89.3% vs Haddad 10.7%
- **MG**: Zema 100%
- **RJ**: Castro 100%
...
1. **Pesquisas T-30 sintéticas**: o dataset `governadores_br_historico.csv` usa polls T-30 plausíveis para ampliar cobertura de cenário. Ele fica separado dos 394 eventos reais usados no headline year-fold.
...
scripts/evolve_political_model.py               # evolução com gates de promoção
...
data/political_evolution.json                   # relatório evolutivo
data/political_stats_v2.json                    # indicadores de rigor
docs/ONBOARDING_POLITICAL.md                    # este documento
```

Total: 14 arquivos principais. Modificado: `main.py` (registro `politica_router`).
...
| 2016 | 20 | 0.0830 | [0.0307, 0.1389] | 0.857 | [0.700, 1.000] |
| 2018 | 70 | 0.1839 | [0.1766, 0.1910] | 0.986 | [0.957, 1.000] |
| 2020 | 30 | 0.0170 | [0.0070, 0.0312] | 1.000 | [1.000, 1.000] |
| 2022 | 120 | 0.0843 | [0.0707, 0.0980] | 1.000 | [1.000, 1.000] |
| 2024 | 68 | 0.1076 | [0.0836, 0.1337] | 0.897 | [0.809, 0.971] |
...
| N | 394 |
| DM statistic | -4.92 |
| p-value (two-sided) | 8.6e-7 |
| mean loss diff | -0.0324 |
...
| 2022 | 120 | 0.0841 | 0.2500 | 0.2500 | 0.0843 |
| 2024 | 68 | 0.0327 | 0.1805 | 0.2500 | 0.1072 |
...
python3 scripts/smoke_political.py   # 35/35 passing
```
