...
def main():
    ...
    parser.add_argument("--metric", choices=["brier", "accuracy"], default="brier",
                        help="Onda 174: otimizar accuracy (max) ou brier (min, default)")
    ...
    if args.metric == "accuracy":
        # values stored são 1-acc (lower=better), display accuracy
        bb_acc = 1 - bb if bb else 0
        bt_acc = 1 - bt if bt else 0
        print(f"Baseline accuracy: {bb_acc*100:.1f}%")
        print(f"Best accuracy: {bt_acc*100:.1f}%")
    else:
