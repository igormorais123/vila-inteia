...
# Desabilitar OmniRoute nos testes (evitar timeouts de 30s)
...
def test_campus_distancia():
    ...
    dist2 = calcular_distancia("agora", "arena_debates")
...
def test_rascunho():
    ...
    # esta_dormindo depende do estado inicial (pode ser True se hora nao definida)
...
def test_simulacao_init():
    ...
    # 10 + especiais extras (Diabob, Jesus, Helena, Sun Tzu)
    assert_true(len(sim.personas) >= 10, f"Poucos agentes: {len(sim.personas)}")
